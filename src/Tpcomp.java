...
public void charger() throws FileNotFoundException  {
	...
	public String type_numero(String chaine) {
		...
		while (i < chaine.length()) {
			if (chiffre(chaine, i)) pos++;
			else if(point & chaine.charAt(pos)=='.') {
				pos++;
				point = false;
			}
			i++;
		...
		if (pos == chaine.length() && !chaine.contains(".")) return "Nombre entier";
		else if (pos == chaine.length() && !point) return "Nombre reel";
		return null;
				...
				if(mot[i].equals("Int_Number")){ 
					...
					if(mot[i].equals(":")){
						...
						if(ident(mot[i]) != null){
							...
							if(mot[i].equals(";;")) return "Declaration de "+k+" variables int";
						}
					}
